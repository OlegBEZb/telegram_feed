...
from telethon.errors.rpcerrorlist import ChannelPrivateError, UsernameNotOccupiedError, MessageIdInvalidError
    ...
    for msg in reversed(msg_list):  # starting from the chronologically first
        ...
        if msg.grouped_id is not None:  # the current message is a part of a group
            if msg.grouped_id == last_grouped_id:  # extending the same group
                ...
                logger.log(5, f"Group {msg.grouped_id} has one more message to be sent. Total size: {len(grouped_msg_ids)}")
            else:
            ...
            logger.log(5, f"Non-grouped messages list is extended. Total size: {len(msg_non_grouped)}")
        ...
        try:
            ...
            if messages is not None:
                ...
                for dst_ch in dst_ch_list:
                    messages_checked_list = select_messages_for_dst_channel(msg_list, dst_ch, recommender, user_client)

                    if dst_ch.is_public:
                        # may be utilised with an "empty" bot as link is enough?
                        forward_msgs(client=bot_client, peer=src_ch.link, msg_list=messages_checked_list,
                ...
                with user_client:
                    user_client.send_read_acknowledge(src_ch.link, msg_list)
                    logger.debug(f"Channel {src_ch.link} is marked as read for me({client.get_me().id})")
...
def select_messages_for_dst_channel(msg_list: List[Message], dst_ch: Channel,
...
if __name__ == '__main__':
    ...
    forwarding_bot_path = os.path.join(get_project_root(), 'src/bot_for_feed')
