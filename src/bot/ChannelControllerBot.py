...
class Bot(telepot.helper.ChatHandler):
    ...
    def on_chat_message(self, msg):
        ...
        if ((config.admin_chat_id == None and config.channel_id == None) or
            ...
            # bot commands (details in README)
            if command[0][0] == "/":
                if len(command) == 1:
                    ...
                    if command[0] == '/add':
                        new_channel = check_channel_correctness(command[1])
...
if __name__ == '__main__':
    logging.basicConfig(filename="ChannelControllerBot.log",
